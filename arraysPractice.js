...
var reversedLooper = function(letters) {
  for (i = letters.length - 1; i > letters.length - letters.length - 1; i--) {
    alert(letters[i]);
  }
}
...
var removeItem = function(myGroceryList, remove) {
  for (var i = 0; i < myGroceryList.length; i++) {
    if (remove === myGroceryList[i]) {
      return myGroceryList.splice(i--, 0);
    }
  }
}
...
//removeItem('chips') --> ['pizza', 'hotpockets', 'MtnDew', 'corndogs']; **missing first argument**
...
var both = function (arr1, arr2) {
  var newArr = [];
  for (var i = 0; i < longest(arr1, arr2).length; i++) {
    for (var y = 0; y < longest(arr1, arr2).length; y++) {
      if (arr1[i] === arr2[y]) {
        newArr.push(arr1[i]);
        break;
      }
